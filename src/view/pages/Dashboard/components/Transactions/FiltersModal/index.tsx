      ...
      <div className="space-y-2 mt-2">
        <span className='text-lg tracking-[-1px] font-bold text-gray-800'>Conta</span>
        {
          mockedAccounts.map(account => (
            <button onClick={() => handleSelectBankAccount(account.id)} className={cn('p-2 rounded-2xl w-full text-left text-gray-800 hover:bg-gray-50 transition-colors',
            selectedBankAccountId === account.id && 'bg-gray-200')} key={account.id}>
              {account.name}
            </button>
          ))
        }
